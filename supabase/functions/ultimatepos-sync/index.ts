...
async function testConnection(supabase: any, _body: any) {
  ...
  try {
    ...
    const response = await fetch(`${baseUrl}/api/connector/api/business-details`, {
      headers: {
...
async function pushOrder(supabase: any, body: any) {
  ...
  try {
    ...
    const response = await fetch(`${baseUrl}/api/connector/api/sell`, {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${token}`,
...
async function fetchProducts(supabase: any, _body: any) {
  ...
  try {
    ...
    const response = await fetch(
      `${baseUrl}/api/connector/api/product?business_id=${config.business_id || 1}&location_id=${config.location_id || 1}`,
      {
        headers: {
          "Authorization": `Bearer ${token}`,
          "Accept": "application/json",
